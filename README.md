...
다크 헤더·히어로 + 라이트 본문. 화면 구성은 다음과 같다.

```
[다크] Header → Hero(오늘 1위 대형 + 원형 흥행 온도 뱃지)
[라이트] Summary 3카드 → Ad A → 박스오피스 TOP 10 → Methodology → Footer
```
...
| 본문 | SUIT — 300 / 400 / 600 |
| 액센트 | `#e50914` (채움) / `#c9070f` (라이트 배경 텍스트) / `#ff4b55` (다크 배경 텍스트) |
| 상태색 | 상승 레드 · 하락 블루 · 신규 앰버 · 유지 그레이 (기호 병기) |

- **PC 는 폭을 제한하지 않고 화면을 다 쓴다.** 공통 좌우 여백은 `.gutter` 유틸리티가
  320 / 640 / 1024 / 1536px 단계로 넓힌다.
- TOP 10 은 **포스터를 고르면 위쪽 대형 카드가 바뀌는** 구조다. 포스터 줄은 `role="tablist"`,
  대형 카드는 `role="tabpanel"` 이며 ←/→/Home/End 와 좌우 화살표 버튼으로 이동한다.
- 대형 카드 10개를 모두 렌더하고 선택되지 않은 것은 `hidden` 으로 둔다.
  검색봇이 JavaScript 없이도 TOP 10 의 영화명과 지표를 읽어야 하기 때문이다(PRD 30).
- 스테이지는 `grid-template-areas` 로만 재배치한다. DOM 은 하나다.
- 포스터 줄은 모바일에서 줄 안에서만 가로 스크롤하고, PC 에서는 5열 그리드로 폭을 채운다.
  페이지는 320px~1920px 어디서도 가로 스크롤이 없다.

### 지표 표기 원칙

Methodology 섹션은 `lib/heat-score.ts` 의 **실제 배점과 일치**해야 한다.
현재 순위 50 / 순위 모멘텀 20 / 관객 모멘텀 25 / 신규 진입 5.

예매율, 관객 평점, SNS 반응, 검색 트렌드 등은 KOBIS·KMDb 로 조회할 수 없으므로
계산에 쓰이지 않으며 화면에도 표기하지 않는다. 배점을 바꾸면 이 섹션도 함께 고친다.
